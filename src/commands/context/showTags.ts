...
export async function execute(
  ...
  try {
    ...
    const categorizedTags = ValidationService.categorizeTags(
      post.tag_string,
      post.tag_string_artist
    );
    const totalTags = post.tag_string
      .split(" ")
    ...
    const embed = new CustomEmbed("info")
      .withCommandInfo(
        `Tags for Post #${post.id}`,
        `${CONFIG.API.BASE_URL}/posts/${post.id}`
      )
    ...
    if (categorizedTags.general.length > 0) {
      const remainingSpace =
        TAG_LIMIT -
        categorizedTags.artist.length -
        categorizedTags.character.length -
        categorizedTags.copyright.length;
      const displayTags = categorizedTags.general.slice(
        0,
        Math.max(remainingSpace, 10)
      );
      embed.addFields({
        name: "🏷️ General Tags",
    ...
    const displayedTags = Object.values(categorizedTags).reduce(
    ...
    if (totalTags > displayedTags) {
      embed.addFields({
        ...
        value: `And ${format.bold(
          (totalTags - displayedTags).toString()
        )} more tags. Visit the ${format.link(
          "Danbooru page",
          `${CONFIG.API.BASE_URL}/posts/${post.id}`
        )} to see all tags.`,
        inline: false,
